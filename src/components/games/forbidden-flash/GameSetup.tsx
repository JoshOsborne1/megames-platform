...
import { Difficulty } from "@/lib/games/forbidden-flash/types";
import { Users, Zap, ShieldAlert, Trophy, Trash2, Plus } from "lucide-react";
  ...
  return (
    <div className="max-w-2xl w-full bg-[#1a0f2e]/80 backdrop-blur-xl border-2 border-white/10 rounded-3xl p-8 shadow-2xl">
      ...
      <div className="space-y-8">
        ...
        <section>
          <div className="flex items-center gap-2 mb-4 text-[#00f5ff]">
            <Zap className="w-5 h-5" />
            <h3 className="font-display font-bold text-xl uppercase tracking-wider">Difficulty</h3>
          </div>
          <div className="grid grid-cols-3 gap-4">
            {(["easy", "medium", "hard"] as Difficulty[]).map((d) => (
              <button
                ...
                className={`py-4 rounded-2xl font-display font-black text-sm uppercase transition-all border-2 ${
                  difficulty === d
                    ? "bg-[#00f5ff] text-[#1a0f2e] border-[#00f5ff] shadow-[0_0_20px_rgba(0,245,255,0.4)] scale-105"
          ...
          <p className="mt-3 text-[10px] font-space text-white/30 text-center uppercase tracking-widest">
            {difficulty === "easy" && "Avoid 2 forbidden words per card"}
            {difficulty === "medium" && "Avoid 3 forbidden words per card"}
            {difficulty === "hard" && "Avoid all 4 forbidden words per card"}
          </p>
        </section>
        ...
        <motion.button
          ...
          whileTap={{ scale: 0.98 }}
          onClick={() => onStart(playerNames, difficulty)}
