...
import { Menu, X, Search, Trophy, Zap, Star, Maximize2, Minimize2, User } from "lucide-react";
                      ...
                      <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
                        {getAvatarUrl() ? (
                          <img
                            src={getAvatarUrl()!}
                            alt="Profile"
                            className="w-10 h-10 rounded-full border-2 border-[#8338ec] hover:border-[#ff006e] transition-colors"
                          />
                        ) : (
                          <div className="w-10 h-10 rounded-full bg-gradient-to-r from-[#ff006e] to-[#8338ec] flex items-center justify-center text-white font-bold border-2 border-transparent hover:border-[#00f5ff] transition-colors">
                            {getUserInitial()}
                          </div>
                        )}
                      </motion.div>
                    ...
                    <div className="flex gap-3 pt-4 px-4 border-t border-[#8338ec]/30">
                      {user ? (
                        <Link href="/profile" className="flex-1" onClick={() => setMobileMenuOpen(false)}>
                          <Button
                            variant="outline"
                            className="w-full border-[#8338ec] text-white font-display py-4 touch-manipulation min-h-[44px] flex items-center justify-center gap-2"
                          >
